...
defined('PATH_CACHE') or die('Restricted access');
 ...
 * @subpackage cake.cake.libs.cache
 */
class identityCacheFile extends identityCacheEngine
{
    /**
     ...
     *                prefix = string prefix for filename, default => identity_
    ...
    {
	if (!is_dir("/tmp/api-cache/identity"))
		mkdir("/tmp/api-cache/identity", 0777, true);
        parent::init($settings);
        $defaults = array('path' => "/tmp/api-cache/identity" , 'extension' => '.php' , 'prefix' => 'identity_' , 'lock' => false , 'serialize' => false , 'duration' => 31556926);
        $this->settings = array_merge($defaults, $this->settings);
        if (!isset($this->file)) {
            include_once PATH_CACHE . _DS_ . 'class' . _DS_ . 'file' . _DS_ . 'identityfile.php';
            $this->file = identityFile::getHandler('file', $this->settings['path'] . '/index.html', true);
        }
