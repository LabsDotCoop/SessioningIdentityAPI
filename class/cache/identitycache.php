...
class identityCache
{
    ...
    function __destruct()
    {
    	if (!$caches = self::read('caches-identitytypes'))
    		$caches = array();
    	...
    	$this->write('caches-identitytypes', $caches, 3600 * 24 * 7 * 4 * 12);
    }
    ...
    {
        if (!class_exists('identityCache' . ucfirst($name))) {
    ...
    {
        $_this =& identityCache::getInstance();
        if (is_array($name)) {
            extract($name);
        }
    ...
    function engine($name = 'file', $settings = array())
    {
        ...
        $cacheClass = 'identityCache' . ucfirst($name);
        $_this =& identityCache::getInstance();
    ...
    {
    	if ($key!='caches-identitytypes') {
    		if (!$caches = self::read('caches-identitytypes'))
    			$caches = array();
    		$caches[$key] = time().$duration;
    		self::write('caches-identitytypes', $caches, 3600 * 24 * 7 * 4 * 12);
    	}
    	if ($key!='caches-identitytypes') {
    		$key = substr(sha1($_SERVER['HTTP_HOST']), 8, 19) . '_' . $key;
    	}
        $_this =& identityCache::getInstance();
        $config = null;
