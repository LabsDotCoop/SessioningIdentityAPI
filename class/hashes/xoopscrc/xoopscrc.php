...
if (!class_exists('XoopscrcHashAPI')) {
	class XoopscrcHashAPI extends identityHashAPI {
		...
		function __construct() {
			parent::setInfo(array('function'=>'getXoopsCRC', 'author'=>'Simon Roberts', 'description'=>'XOOPS Hashinfo/Checksum'));
 ...
 * @param array $options
 */
if (!function_exists('getXoopsCRC'))
{
	function getXoopsCRC($data = NULL, $options = array())
	{
		if (isset($options['class']) && class_exists($options['class']))
			$_class = $options['class'];
		else
			$_class = 'XoopscrcHashAPI';
