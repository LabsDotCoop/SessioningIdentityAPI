...
include_once dirname(__FILE__).'/qcp135.class.php';

if (!class_exists('Qcp135HashAPI')) {
	class Qcp135HashAPI extends identityHashAPI {
		...
		function __construct() {
			parent::setInfo(array('function'=>'getQcp135', 'author'=>'Simon Roberts', 'description'=>'XOOPS Hashinfo/Checksum'));
 ...
 * @param array $options
 */
if (!function_exists('getQcp135'))
{
	function getQcp135($data = NULL, $options = array())
	{
		if (isset($options['class']) && class_exists($options['class']))
			$_class = $options['class'];
		else
			$_class = 'Qcp135HashAPI';
